...
#         # num_features = 128
#         # print("r=",r)
#         x = basic_block1(x,num_features,2,2**r,name=f'block_{k+1}')
#         # print(x.shape)
#         k += 1
...
def basic_block1(x,num_features,size,rate,name):
    ...
    #     original_x = Conv1D(num_features,size,padding='same',use_bias=True,
    #     kernel_regularizer=l2(WEIGHT_DECAY),name=f'conv_{name}')(x)
    #     original_x = BatchNormalization()(original_x)
    # else:
    # 残差
    original_x = x
...
def proposed_CNN(input_shape,num_class):
    ...
    for i in range(NUM_BLOCKS):
        ...
        x = Activation('relu',name=f'relu0_{k}')(x)
        # for r in range(4):
        x = basic_block1(x,num_features,kernel_size,kernel_size**r,name=f'block_{k+1}')
        # print(x.shape)
        k += 1
